...
const Men = () => {
  ...
  const MenProducts = APIproducts.filter((product) => product.category === "Men");
  ...
  useEffect(() => {
    applyFilters();
  }, [priceRange, selectedSizes, MenProducts]);
  ...
  return (
    <div className="Men-container mb-10">
      ...
      <div className='w-full h-fit flex flex-col gap-3 mt-10 px-3'>
        ...
        {(priceRange.min !== 0 || priceRange.max !== 1000 || selectedSizes.length > 0) && (
          <div className="showing-filter-values flex flex-col md:flex-row items-center gap-3">
            <p>{filteredProducts.length > 0 ? `${filteredProducts.length} products found` : `${MenProducts.length} products found`}</p>
