...
const Accessories = () => {
  ...
  const Accessories = APIproducts.filter((product) => product.category === "Accessories");
  ...
  useEffect(() => {
    applyFilters();
  }, [priceRange, selectedSizes, Accessories]);
  ...
  const applyFilters = () => {
    let filtered = Accessories.filter((product) => {
  ...
  return (
    <div className="Accessories-container mb-10">
      ...
      <div className='w-full h-fit flex flex-col gap-3 mt-10 px-3'>
        ...
        {(priceRange.min !== 0 || priceRange.max !== 1000 || selectedSizes.length > 0) && (
          <div className="showing-filter-values flex flex-col md:flex-row items-center gap-3">
            <p>{filteredProducts.length > 0 ? `${filteredProducts.length} products found` : `${Accessories.length} products found`}</p>
