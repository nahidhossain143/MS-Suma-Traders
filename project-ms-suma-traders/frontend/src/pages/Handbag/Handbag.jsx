...
const Handbag = () => {
  ...
  const Handbags = APIproducts.filter((product) => product.category === "Handbags");

   useEffect(() => {
      if (APIproducts.length > 0) {
        setFilteredProducts(Handbags); 
        setIsLoading(false);
        resetFilters('all'); 
      }
    }, [APIproducts]);
    ...
    useEffect(() => {
      applyFilters();
    }, [priceRange, selectedSizes, Handbags]);
  ...
  const applyFilters = () => {
    let filtered = Handbags.filter((product) => {
  ...
  return (
    <div className="handbags-container mb-10">
      ...
      <div className='w-full h-fit flex flex-col gap-3 mt-10 px-3'>
        ...
        {(priceRange.min !== 0 || priceRange.max !== 1000) && (
          <div className="showing-filter-values flex flex-col md:flex-row items-center gap-3">
            <p>{filteredProducts.length > 0 ? `${filteredProducts.length} products found` : `${Handbags.length} products found`}</p>
        ...
        <div className={`filter-items ${openFilter ? 'showFilter' : ''}`}>
          ...
          <div className={`Product-categories mt-5 w-full pb-5 ${expanded.productCategories ? 'expanded' : 'collapsed'}`}>
            ...
            <div className="filter-links">
              <NavLink to={"/NewArrival"}>
                <p className="mt-6 hover:text-red-600">New Arrival</p>
              </NavLink>
              <NavLink to={"/Men"}>
                <p className="mt-2 hover:text-red-600">Men</p>
              </NavLink>
              <NavLink to={"/Women"}>
          ...
          <div className={`product-size-filter  mt-5 w-full pb-5 ${expanded.Size ? 'expanded' : 'collapsed'} `}>
            ...
            <div className="filter-sizes mt-5 pb-5">
              <p>Sizes are not available in this category</p>
            </div>
          </div>
        </div>
      </div>
