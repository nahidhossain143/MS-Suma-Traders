...
import { faMinus, faPlus, faTimes, faChevronLeft } from '@fortawesome/free-solid-svg-icons';
...
const Fashion = () => {
  ...
  const NewArrival = APIproducts.filter((product) => product.category === "NewArrival");

  useEffect(() => {
    if (APIproducts.length > 0) {
      setFilteredProducts(NewArrival); 
      setIsLoading(false);
      resetFilters('all');
    }
  }, [APIproducts]);
  ...
  useEffect(() => {
    applyFilters();
  }, [priceRange, selectedSizes, NewArrival]);
  ...
  const applyFilters = () => {
    let filtered = NewArrival.filter((product) => {
  ...
  return (
    <div className="NewArrival-container mb-10 h-fit w-full ">
      ...
      <div className='w-full h-fit flex flex-col gap-3 mt-10 px-3'>
        ...
        {(priceRange.min !== 0 || priceRange.max !== 1000 || selectedSizes.length > 0) && (
          <div className="showing-filter-values flex flex-col md:flex-row items-center gap-3">
            <p>{filteredProducts.length > 0 ? `${filteredProducts.length} products found` : `${NewArrival.length} products found`}</p>
